...
import { parsePrismaSchema, PrismaSchema, SchemaDeclaration, Config, formatAst } from "@loancrate/prisma-schema-parser";
import { DataSourceConfig } from "../services/DB";
import { prismaSqliteURLToFilePath } from "./readDataSourceConfig";
import { ConfigSchema } from "../config/config.type";


function isNonClientGenerator(decl: SchemaDeclaration): boolean {
  return decl.kind === "generator" && !(decl.members?.some(member => 
    member.kind === "config" &&
...
function updateGenerator(ast: PrismaSchema, clientOutputPath: string): PrismaSchema {
  ...
  let generatorOutputAttribute = generator.members.find(attr =>
    attr.kind === "config" &&
    attr.name?.value === "output"
  ) as Config | undefined;
...
function updateDatasource(
  ...
  let dataSourceUrlAttribute = dataSourceDecl.members.find(attr =>
    attr.kind === "config" &&
    attr.name?.value === "url"
  ) as Config | undefined;
...
/// Creates a temporary Prisma schema file for generating
/// the client for a migration. The schema is based on the
/// source schema file, but with updated generator and datasource
/// blocks.
